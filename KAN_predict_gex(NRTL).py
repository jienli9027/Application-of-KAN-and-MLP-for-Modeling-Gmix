...
for ablation_mask in range(1):
    ...
    for g in range (1):
        ...
        mat=[8,0]
        path = f'C:/Users/Wonglab/Desktop/WONG/backup_result/PINN/KAN_gmix_RG_val_training/KAN_NRTL_{mat}_H{ablation_mask}_derivative_calculation_d1_d2_phasediagram_opwe'
        dataname='NRTL_Gex_data_RG_Phenol+octane_difference2_1overT'
        ...
        test_data_RG = filtered_data[~filtered_data["T"].isin(selected_T_R_levels)]
        
        
        test_data_RG = test_data_RG.reset_index(drop=True)
        test_T_R= test_data_RG["T"].drop_duplicates().reset_index(drop=True)
        x_test = test_data_RG["x"].drop_duplicates().reset_index(drop=True)

        

        val_data_RG = data[(data['T'] == 1.724137931)].reset_index(drop=True)
        ...
        num=[11,21,41,51,101,126]#251,501,1001,201,
        step_loss_unprune=[]
        for i in range (1):
            for j in range (1):
                m = 101
                ...
                T_num = 101
                index_step = len(T_R_flat) // (T_num-1)  # 例如，如果想近似取出 10 個點
                if index_step == 0:
                    index_step = 1 # 避免除以零
                selected_T_R_levels = equidistant_sampling_by_index(T_R_flat, index_step)
                ...
                for _ in pbar:
                    def closure():
                        ...
                        pred_d2 = torch.autograd.grad(outputs=pred, inputs=dataset['train_input'], grad_outputs=grad_pred, create_graph=True)[0][:,0]*(-1/(2*T_R_max)*standarize.unmapping_T(dataset['train_input'][:,0],T_R_max,T_R_min)**2)
                        ...
                        loss = (
                            loss_T +
                            ablation_mask[0] * torch.exp(-log_vars[0]) * loss_d1 +
                            ablation_mask[1] * torch.exp(-log_vars[1]) * loss_d2 +
                            ablation_mask[2] * torch.exp(-log_vars[2]) * loss_d3
                            )
                        loss.backward()
                        return loss
                    ...
                    pred_val_d2 = torch.autograd.grad(outputs=pred_val, inputs=dataset['test_input'], grad_outputs=grad_pred_val, create_graph=True)[0][:,0]*(-1/(2*T_R_max)*standarize.unmapping_T(dataset['test_input'][:,0],T_R_max,T_R_min)**2)
                    ...
                    loss_val =  loss_val_T  # + torch.exp(-log_vars[0]) *loss_val_d1 + torch.exp(-log_vars[1]) *loss_val_d2 + torch.exp(-log_vars[2]) *loss_val_d3
                ...
                gradients2=torch.autograd.grad(outputs=output.view(-1, 1), inputs=dataset_test['test_input'], grad_outputs=torch.ones(output.size(), device=device), create_graph=True)[0][:,0]*(-1/(2*T_R_max)*standarize.unmapping_T(dataset_test['test_input'][:,0],T_R_max,T_R_min)**2)
    ...
    # 
                # pred_se_train=model(dataset['train_input']).reshape(-1)
                ...
                gradients2_train=torch.autograd.grad(outputs=output_train.view(-1, 1), inputs=dataset['train_input'], grad_outputs=torch.ones(output_train.size(), device=device), create_graph=True)[0][:,0]*(-1/(2*T_R_max)*standarize.unmapping_T(dataset['train_input'][:,0],T_R_max,T_R_min)**2)
                acoe1_r_train=acoe1(train_input,T_R_max,T_R_min)
                # acoe2_p_train,acoe2_p_dd_train=acoe2_pred_dd(dataset['train_input'],Gex_train,output_train)
                ...
                gradients2_val=torch.autograd.grad(outputs=output_val.view(-1, 1), inputs=dataset['test_input'], grad_outputs=torch.ones(output_val.size(), device=device), create_graph=True)[0][:,0]*(-1/(2*T_R_max)*standarize.unmapping_T(dataset['test_input'][:,0],T_R_max,T_R_min)**2)
                ...
                with open(f'{path}/log_vars.txt', 'w') as f:
                    f.write(','.join(map(str, log_vars.flatten().cpu().detach())))
