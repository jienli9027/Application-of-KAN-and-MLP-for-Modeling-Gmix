...
def train(model, dataloader, criterion, optimizer, device, sacle_data,log_vars,ablation_mask):
    ...
    for batch in tqdm(dataloader, desc='Training', ncols=100):
        ...
        loss = ablation_mask[0]*torch.exp(-log_vars[1]) * loss_d + ablation_mask[1]*torch.exp(-log_vars[2]) * loss_dT + 0*target_loss + ablation_mask[2]*torch.exp(-log_vars[3]) *loss_d2
...
for ablation_mask in ablation_masks_list:
    for l in range(1):
        print("Training with ablation_mask =", ablation_mask)
        path = f'C:/Users/Wonglab/Desktop/WONG/backup_result/PINN/KAN_gmix_RG_val_training/DNN_v2_NRTL_lr=0.0001_[500]'
        if not os.path.isdir(path):
            os.mkdir(path)
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        np.set_printoptions(threshold=np.inf)
        dataname='NRTL_Gex_data_RG_Phenol+octane_difference2'
        ...
        for i in range (1):
            for j in range (1):  
                ...
                index_step = len(T_R_flat) // (m-1)  # 例如，如果想近似取出 10 個點
                if index_step == 0:
                    index_step = 1 # 避免除以零
                ...
                model = torch.load(f'{path}/DNN_NRTL_Gex_data_RG_Phenol+octane_difference2_all.pkl')
